...
export default function BlockNoteEditorComponent({ pageId }: EditorProps) {
    ...
    // Text selection: skip page blocks when dragging
    // When user drags to select text and hits a page block, extend selection to next regular block
    useEffect(() => {
        ...
        const handleSelectionChange = () => {
            ...
            const currentSelectionString = selection.toString();
            if (currentSelectionString === lastSelectionString) return;
            lastSelectionString = currentSelectionString;

            const range = selection.getRangeAt(0);

            // Find the block elements for start and end
            ...
            // Check if selection ends in a page block
            const endIsPageBlock = endBlock.querySelector('a[href^="/page/"]') !== null;
            const startIsPageBlock = startBlock.querySelector('a[href^="/page/"]') !== null;

            // If selection ends in page block, try to extend to next non-page block
            if (endIsPageBlock && !startIsPageBlock) {
                isAdjusting = true;

                // Find next non-page block after endBlock
                let nextBlock = endBlock.nextElementSibling as HTMLElement | null;
                while (nextBlock) {
                    if (nextBlock.classList.contains('bn-block-outer') && nextBlock.hasAttribute('data-id')) {
                        const isPageBlock = nextBlock.querySelector('a[href^="/page/"]') !== null;
                        if (!isPageBlock) {
                            // Found a non-page block - extend selection to its first text node
                            const textContent = nextBlock.querySelector('.bn-inline-content');
                            if (textContent) {
                                ...
                                if (firstTextNode) {
                                    try {
                                        ...
                                        // Selection adjustment failed, ignore
                                    }
                                }
                            }
                            break;
            ...
            // If selection starts in page block (dragging up), extend to previous non-page block
            if (startIsPageBlock && !endIsPageBlock) {
                isAdjusting = true;

                // Find previous non-page block before startBlock
                let prevBlock = startBlock.previousElementSibling as HTMLElement | null;
                while (prevBlock) {
                    if (prevBlock.classList.contains('bn-block-outer') && prevBlock.hasAttribute('data-id')) {
                        const isPageBlock = prevBlock.querySelector('a[href^="/page/"]') !== null;
                        if (!isPageBlock) {
                            // Found a non-page block - extend selection from its last text node
                            const textContent = prevBlock.querySelector('.bn-inline-content');
                            if (textContent) {
                                const walker = document.createTreeWalker(textContent, NodeFilter.SHOW_TEXT);
