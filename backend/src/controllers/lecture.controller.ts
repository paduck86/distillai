  ...
  try {
    ...
    if (lecture.sourceType === 'youtube' && lecture.sourceUrl) {
      ...
      try {
        ...
        // 텍스트 기반 요약 수행
        result = await aiService.summarizeFromTranscriptWithCategory(transcript.text, lecture.title);
      } catch (ytError) {
        ...
        if (errorMessage === 'NO_CAPTIONS_AVAILABLE') {
          ...
          try {
            ...
            // 텍스트 기반 요약 수행
            result = await aiService.summarizeFromTranscriptWithCategory(transcript, lecture.title);
          } catch (dlError) {
      ...
      // Process with OpenAI (요약 + AI 카테고리 추출)
      result = await aiService.summarizeWithCategoryExtraction(audioUrl, lecture.title);
    }
    ...
    // Update lecture with results (AI 카테고리 정보 포함)
