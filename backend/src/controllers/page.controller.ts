...
export async function uploadImageHandler(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (!distillationId) {
      throw new ValidationError('distillationId is required');
    }

    // Verify user owns the distillation
    ...
    // Get public URL
    const publicUrl = await storageService.getImagePublicUrl(imagePath);
